function PSD_func(PSD)
    ...
    % end
    PSD = PSD/L/(2*J+1)/T; LPSD = length(PSD);
    ...
    ylabel('$\Psi_x(\Omega)$','FontSize',16,'interpreter','latex');drawnow
    fT = linspace(-T*fs/2,T*fs/2,LPSD); % normalized freq axis
    Area = sum(PSD)*(fT(2)-fT(1)); CTFT_PSD = PSD/Area;
    ...
    figure(4);semilogy(fT,CTFT_PSD,'k-','linewidth',1.5);set(gca,'FontSize',14);
    xlabel('$-Tf_s\,/\,2 \le Tf\le Tf_s\,/\,2$','FontSize',16,'interpreter','latex')
    ylabel('$\Psi_x(f)$ [dB]','FontSize',16,'interpreter','latex');ylim([1e-3 1]);
    xlim(T*[-fs/2 fs/2]);title('Log PSD vs. $Tf$ [Normalized and two-sided]',...
    'FontSize',14,'interpreter','latex');grid on;axis tight; drawnow
end